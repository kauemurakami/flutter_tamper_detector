...
bool installedFromStore = await FlutterTamperDetector.isInstalledFromStore();
```
...
Only for `isInstalledFromStore` we have a different parameter that is similar to the previous one but this time, we want to take action if the return is false, and not true, so we use `exitProcessIfFalse: true` if the app was not installed directly from the store. (in debug this will always return false)
```dart
...
bool installedFromPlayStore = await FlutterTamperDetector.isInstalledFromStore(exitProcessIfFalse: true);
```
